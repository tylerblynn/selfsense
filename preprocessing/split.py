...
class NParrays():
    ...
    def finalize(self):
        self.x = np.delete(my_X, (0), axis=0) 
        self.y = np.delete(my_y, (0), axis=0) 
        self.sub = np.delete(my_sub, (0), axis=0)
    ...
    def trainTestSplit(self, validation, testSize):
        x_test, x_train, y_test, y_train = sklearn.train_test_split(self.x, self.y, test_size = testSize, random_state = 1)
