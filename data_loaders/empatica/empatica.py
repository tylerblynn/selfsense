"""this is the library of functions/ class designed to get data gathered from the empatica 
into the standard format for merging"""
...
class Empatica():
    ...
    def __init__(self, subject):        
        ...
        self.freqDict = {"accel" : 32, "bv_pulse" : 64, "electro_dermal" : 4, 
                     "heart_rate" : 1, "temp" : 4}
        self.sensorNames = ["accelerometer", "bvp","eda","hr","temp"]
    ...
    def frequencyGeneration(self):
        if "accel_x" or "accel_ttl" not in self.mainFrame.columns:
            self.acc = False
        if "bv_pulse" not in self.mainFrame.columns:
            pass
    ...
    def makeSensorList():
        #retrive a binary list from user for active sensors
        sensorList = []
        for i in range(len(self.sensorList)):
            print("Enter 1 if you will be including "+ self.sensorList[i] +" and 0 if not")
            answer = input()
            sensorList.append(answer)
        return sensorList

    def makeSensorFields():
        self.acc = self.sensorList[0]
        self.bvp = self.sensorList[1]
        self.eda = self.sensorList[2]
        self.hr = self.sensorList[3]
        self.temp = self.sensorList[4]
    ...
    def resampleMF(self, targetRate): 
